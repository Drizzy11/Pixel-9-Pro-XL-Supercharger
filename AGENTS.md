...
- `site/`: public English/Spanish website, separate from the Android WebUI.
  `site/index.html` is the English landing-page source; other HTML pages are
  generated by `scripts/build_site.py`. See `docs/WEBSITE.md` before editing.
...
- Use Python 3.10+ and Node 24. Run `python3 scripts/build_site.py` from the
  committed public snapshot, then `python3 scripts/check_site.py`; preview with
  `python3 scripts/serve_site.py`.
- The standalone website gate checks links, languages, JavaScript syntax,
  interactions, requirements, ZIP hashing, issue drafts, release data, and atomic
  rendering. It runs `scripts/site_regression.test.mjs`,
  `scripts/site_tools.test.mjs`, and `scripts/test_site_*.py`.
- Verify changed website flows in a browser at desktop and mobile sizes.
- Keep metadata strictly public; never expose draft releases. The report builder
  only prepares a user-reviewed GitHub form. ZIP verification runs locally in the
  browser. The website has no Android bridge or backend and never ships in module
  ZIPs. Sample dashboard values are not device validation.
